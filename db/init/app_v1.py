...
SELECT
  o.customerid,
  DATE_PART('day','{dt}'::date - MAX(o.invoicedate)) AS recency,
...
if can_do_geography():
    country_sales = pd.read_sql_query(
        "SELECT c.country, SUM(i.linetotal) AS revenue FROM fact_order_item i \
         JOIN fact_order o USING(orderid) \
